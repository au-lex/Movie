...
const SingleMovieDetail = ({
  ...
  const voteCount = convertToShortForm(vote_count)
  return (
    <>
      <TrailerModal
      ...
      <div className="grid lg:grid-cols-3 gap-8 pt-6">
        <div className=" lg:col-span-2">
          <div className=" flex flex-col lg:flex-row items-center justify-start lg:justify-between  ">
            <div className="flex items-center gap-1 text-gray-800 font-bold text-lg lg:text-xl flex-wrap">
              ...
              <p data-testid="movie-release-date">
                {formatUTCDate(release_date)}
              </p>
              <p className=" "> •</p>
              <p className=" ">PG-13 </p>
              <p className=" ">• </p>
              <p data-testid="movie-runtime">{convertToMinutes(runtime)}</p>
            </div>
          ...
          <div className="text-gray-600 lg:text-lg font-medium space-y-6">
            ...
            <p>
              Director : <span className="text-rose">{directors}</span>
            </p>
            <p>
              Writers : <span className="text-rose">{Writers}</span>
            </p>
            <div className="flex items-center gap-1">
              <p>Stars : </p>
              {stars.map((star: any, index: number) => (
                <span key={index} className="text-rose">
                  {index === stars.length - 1 ? star : star + ','}
                </span>
              ))}
            </div>
        ...
        <div className="bg- grid-cols-3 lg:grid-cols-1">
          <div className=" inline-flex items-center justify-end gap-2 float-right pb-4">
            ...
            <p className="text-gray-400 text-lg font-bold ">
              {roundToOneDecimalPlace(vote_average)}{' '}
              <span className="text-gray-900">| {Math.floor(voteCount)}k</span>
            </p>
          </div>
          <Button
            leftIcon={<TicketIcon />}
